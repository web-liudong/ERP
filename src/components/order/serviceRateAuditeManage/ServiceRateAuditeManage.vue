<template>
  <div class="brandBox">
    <div class="buttonBox">
      <el-button size="small" :class="{active:orderStatus==item.status}" v-for="(item,index) in activeList" :key="index" @click="switchStatus(item.status)">
    ...
    <div class="filterBox">
      ...
      <div class="homeBoxBottom" v-show="isShow">
        <!-- <div class="chunk">
          <span>申请单号：</span>
          <el-input placeholder="请输入申请单号" v-model="query.orderCancelReviewId" clearable></el-input>
        </div> -->
        <!-- <div class="chunk">
          <span>申请发起方：</span>
        ...
        <div class="chunk">
          <span>订单号：</span>
          <el-input v-model.number="query.no" placeholder="订单号"></el-input>
        </div>
        <div class="chunk">
          <span>外部订单号：</span>
    ...
    <el-table :data="tableData" border style="width: 100%" size="small" :max-height="dataListHeight">
      ...
      <el-table-column prop="changeReviewNo" label="申请单号" align="center" fixed="left" width="160">
        <!-- <template slot-scope="scope">
          <el-button
            @click="oepnWin('OrderDetail',{ id: scope.row.orderCancelReviewId })"
            type="text"
            size="small"
          >{{scope.row.orderCancelReviewId}}</el-button>
        </template> -->
      </el-table-column>
      ...
      <el-table-column prop="customerOrderNo" label="外部订单号" align="center" width="150"></el-table-column>
      <el-table-column prop="memoOrderNo" label="备注订单号" align="center" width="100" show-overflow-tooltip></el-table-column>
      <el-table-column prop="orderSourceName" label="订单来源" align="center">
        <template slot-scope="scope">
      ...
      <el-table-column prop="projectName" label="项目" align="center" width="140" show-overflow-tooltip>
        <!-- <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
      ...
      <el-table-column prop="customerName" label="客户" align="center" width="140" show-overflow-tooltip>
        <!-- <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
      ...
      <el-table-column prop="staffName" label="业务员" align="center">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
          >{{scope.row.staffName}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="customerUserPhone" label="采购人联系方式" align="center" width="150" show-overflow-tooltip></el-table-column>
      ...
      <el-table-column prop="customerUserDataAuthorityName" label="乙方客户负责人" align="center" width="120">>
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
          >{{scope.row.customerUserDataAuthorityName}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="firstServerName" label="服务方" align="center" show-overflow-tooltip></el-table-column>
      <el-table-column prop="firstServerUserName" label="服务方接单人" align="center" width="120">>
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
          >{{scope.row.firstServerUserName}}</el-button>
        </template>
      </el-table-column>
      <!-- <el-table-column prop="firstServerUserName" label="服务方客户负责人" align="center"></el-table-column> -->
      <!-- <el-table-column prop="secondServerName" label="服务方客户负责人" align="center"></el-table-column> -->
      <el-table-column prop="oldServiceFeeRate" label="改前服务方服务费率" align="center" width="150">>
        <template slot-scope="scope">
          <span>{{scope.row.oldServiceFeeRate*100}}%</span>
        </template>
      </el-table-column>
      <el-table-column prop="newServiceFeeRate" label="改后服务方服务费率" align="center" width="150">>
        <template slot-scope="scope">
          <span style="color:red">{{scope.row.newServiceFeeRate*100}}%</span>
        </template>
      </el-table-column>
      <el-table-column prop="assignUserName" label="修改人" align="center"></el-table-column>
      <el-table-column prop="createTime" label="修改申请日期" align="center" width="120" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>{{scope.row.createTime}}</span>
        </template>
      </el-table-column>
      <el-table-column prop="changeReason" label="更改原因" align="center"></el-table-column>
      <el-table-column prop="assignUserName" label="派单员" align="center">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
          >{{scope.row.assignUserName}}</el-button>
        </template>
      </el-table-column>
      <el-table-column prop="reviewStatus" label="审核状态" align="center" fixed="right" show-overflow-tooltip>
        <template slot-scope="scope">
          <span>{{scope.row.reviewStatus|DD}}</span>
          <!-- <br>
          <span v-if="orderStatus==2">{{scope.row.reviewStatus}}</span> -->
        </template>
      </el-table-column>
      <el-table-column label="操作" align="center" width="150" fixed="right">
        <template slot-scope="scope">
          <el-button
            type="text"
            size="small"
            @click="auditePassRate(scope.row)"
            v-if="scope.row.reviewStatus==0&&$authorities.OrderServiceRateExaminePass"
                 >审核通过</el-button>
          <el-button @click="showDialog('rejectrate',scope.row)" type="text" size="small" v-if="scope.row.reviewStatus==0&&$authorities.OrderServiceRateExamineNoPass">审核驳回</el-button>
          <el-button @click="showDialog('raterecord',scope.row.orderServerId)" type="text" size="small" v-if="$authorities.OrderServiceRateExamineRecord">查看审核记录</el-button>
        </template>
      </el-table-column>
    </el-table>
    <div class="block">
      ...
      ></el-pagination>
       ...
       <rejectrate ref="rejectrate" @callback="dialogCallback"></rejectrate>
...
<script>
import { getRateList ,getRateToAuditeList,getRateAuditeList,getRateRejectList,getRateStatus,RateAuditePass,getUser,checkReview} from "@/api/ordernodraft.js";
import {
  nameList,
  souName,
  simpleList,
...
import { provinces_based_id, subs } from "@/api/basicSupplier";
  ...
  import rejectrate from "../child/reject_rate";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      query: {
        assignUserId: "", //派单员id
        // cancelOriginator: "", //取消发起方
        customerGroupUserDataAuthorityId: "", //集团客户负责人id
        customerId: "", //客户id
        customerOrderNo: "", //外部订单号
        customerUserId: "", //客户采购人id
        customerUserPhone: "", //客户联系方式
        dateType: "", //	日期类型（0：下单日期； 1：期望配送日期； 2：分派日期； 3：完成日期；）
        endTime: "", //操作结束时间
        serverId: "", ///服务方id
        serverUserId: "", //服务方负责人id
        no: null, //订单号
        // orderCancelReviewId: "", //申请单号
        orderSource: "", //订单来源（0：后台，1：接口，2：官网，3：专柜
        orderType: "", //订单类型（0：常规订单，1：赠品订单，2：积分订单）
        partyBId: "", //项目乙方id
        customerUserDataAuthorityId: "", //乙方客户负责人id
        reviewStatus: "", //审核状态 0:未审核，1：已通过，2：已驳回
        projectId: "", //项目id
  ...
  components: {
    // cancelOrder
    rejectrate,raterecord,changeInfo
  },
  created(){
  this.getRateStatus()
  this.changePerson=JSON.parse(window.localStorage.getItem('UserInfo')).realName;
  },
  activated(){
  ...
  methods: {
    ...
    getRateStatus(){
      getRateStatus().then(res=>{
        this.activeList=res.data;
        // this.activeList.unshift(this.activeList.pop());
      })
    },
    //获取表格数据
    getTableData() {
      if(this.orderStatus==-1){
        getRateList(this.query).then(res => {
      ...
      if(this.orderStatus==0){
        getRateToAuditeList(this.query).then(res => {
        this.tableData = res.data.items;
        this.totalPage = res.data.total;
        this.query.pageSize=res.data.pageSize;
        this.query.pageNum=res.data.pageNum;
      ...
      if(this.orderStatus==1){
        getRateAuditeList(this.query).then(res => {
        this.tableData = res.data.items;
        this.totalPage = res.data.total;
        this.query.pageSize=res.data.pageSize;
        this.query.pageNum=res.data.pageNum;
       ...
       if(this.orderStatus==2){
        getRateRejectList(this.query).then(res => {
        this.tableData = res.data.items;
        this.totalPage = res.data.total;
        this.query.pageSize=res.data.pageSize;
        this.query.pageNum=res.data.pageNum;
    ...
    auditePassRate(row){
      this.$confirm('确定已双方沟通完毕，允许当前订单的服务费率按照申请进行修改？','提示',{
         confirmButtonText: '确定',
          cancelButtonText: '取消',
      }).then(()=>{
       RateAuditePass(row.orderServiceFeeRateChangeId).then(res=>{
        if(res.success==true){
            this.$message({
                message:'审核通过',
                type:'success'
    ...
    switchStatus(val) {
      ...
      // this.query.partyBReviewStatus = val;
    ...
    showDialog(type, row) {
      if (type == 'rejectrate') {
        checkReview(row.orderServiceFeeRateChangeId).then(res => {
