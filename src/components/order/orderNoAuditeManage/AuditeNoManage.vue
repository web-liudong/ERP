<template>
  <div class="detaillBox" ref="detaillBox">
    ...
    <div class="detaillTable" v-if="tableData">
      ...
      <el-form :inline="true" :model="tableData" class="demo-form-inline">
        ...
        <el-form-item style="margin-left:60px;" label="配送备注：">
          <el-input v-model="tableData.user" disabled placeholder="" style="width:400px"></el-input>
        </el-form-item>
      </el-form>
    </div>
    <div class="detaillTable2 clear">
      ...
      <div style="margin-top: 10px；margin-bottpm:20px;float:right">
          <span style="margin-right:100px">合计：￥{{total.toFixed(2)}}</span><span></span>
        </div>
        <div class="button-box">
          <el-button type="primary" @click="toAudite()">审核通过</el-button>
          <el-button type="primary" @click="toReject({'id':tableData.orderCancelReviewId})">审核驳回</el-button>
    ...
    <rejectnoauditeorder ref="rejectnoauditeorder" @callback="discallback()"></rejectnoauditeorder>
        <changeinfo :title="title" :des="des" ref="changeinfo" @callback="discallback()"></changeinfo>
        <showGoodsInfo ref="showGoodsInfo"></showGoodsInfo>
  </div>
</template>
<script>
import addhead from "@/components/common/head/head";
import { getCancelDetail,passCancelOrder ,rejectCancelOrder,testRejectCancelOrder,getCancelCheck} from "@/api/ordernodraft.js";
...
export default {
  ...
  methods: {
    ...
    getdata() {
      getCancelCheck(this.orderId).then(res=>{
        if(res.success==true){
          getCancelDetail(this.orderId).then(res => {
          this.tableData = res.data;
          this.goodsList=res.data.orderItemDTOList;   
      });
        }else{
          this.$confirm(res.message, "提示", {
                ...
                .then(res => {
                  // ,to:{name:'OrderNoAuditeManage',params:{isUpdate:true}}
                 this.close({name:'AuditeNoManage', to:{name:this.source,params:{isUpdate:true}}});
                //  window.localStorage.setItem("NOMANAGE_LIST", Date.now())                
                })
                .catch(() => {}); 
        }
      })
    ...
    toAudite(){
        passCancelOrder(this.orderId).then(res=>{
         if(res.data==0){
            this.$message({
                message:'审核通过',
                type:'success'
         ...
         if(res.data==1){
           this.title='变更提示';
           this.des='当前订单状态已发生变化，请刷新数据获取最新状态。';
           this.$refs.changeinfo.init()
         }
         if(res.data==2){
             this.title='提示';
             this.des='当前订单仓库已复核出库，订单不可再更改，只能在订单已完成后进行退货申请，当前只能驳回申请。'
             this.$refs.changeinfo.init();
         }
         if(res.data==3){
             this.title='提示';
         ...
         if(res.data==4){
             this.title='提示';
             this.des='订单已经非整单发货（拆单或自定义发货）。'
             this.$refs.changeinfo.init();
         }
        })
    },
    toReject(row){
     testRejectCancelOrder(row.id).then(res=>{
       if(res.data==0){
         this.$refs.rejectnoauditeorder.init(row);
       }
       if(res.data==1){
           this.title='变更提示';
           this.des='当前订单状态已发生变化，请刷新数据获取最新状态';
           this.$refs.changeinfo.init(row);
       }
       if(res.data==3){
           this.title='提示';
           this.des='当前订单已生成结算单，？？？？？？？？？？？';
