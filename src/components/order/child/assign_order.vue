...
export default {
  ...
  methods: {
    init(row) {
      ...
      this.title = row.status == 0 ? "订单分派" : "订单重新分派";
    ...
    submitForm(name) {
      this.$refs[name].validate(valid => {
        if (valid) {
          allocationOrder(this.form).then(res => {
            this.isShow = false;
            this.$emit("callback");
            this.$message({
              type: "success",
              message: this.form.status == 0 ? "分配订单成功" : "重新分配订单成功"
            });
          });
        } else {
    ...
    reset() {
      ...
      this.remoteOneList = null;
      this.remoteTwoList = null;
