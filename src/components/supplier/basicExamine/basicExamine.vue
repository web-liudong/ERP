...
<template>
  <div class="basicSupplerDetaillBox">
    ...
    <div class="detaillTable" style="border-top: 0px;">
      <div class="detaillTopBox">
        <span><i class="icon iconfont iconshujuliebiao"></i>公司财务信息</span>
      </div>
      <table border="1">
       <tr>
          <th>开票抬头：</th>
    ...
    <div class="detaillTable2 clear">
      ...
      <el-button type="primary" @click="dialogVisible = true" v-if="$authorities.SupplierBasicReview">审核通过</el-button>
      <el-button type="primary" @click="Examback()" v-if="$authorities.SupplierBasicReview">驳回</el-button>
    </div>
...
//  			regionIds
        ...
        this.$refs[f].validate((valid) => {
          if (valid) {
            ...
            updateSubmitReviewSupplier(this.$route.query.detailId,a).then(res=>{
            	if(res.data==1){
            		this.dialogFormVisible=false
            		this.$message({
				          message: '驳回成功',
...
//    	reviewPassSupplier
      ...
      Examgo1(){
      	reviewPassSupplier(this.$route.query.detailId).then(res=>{
      		if(res.data==1){
            		this.dialogVisible = false
            		this.$message({
				          message: '成功',
