    ...
    <div class="editerBrandBox">
      ...
      <div class="pictureBox">
        ...
        <DraggleUpload
          ...
          ref="DraggleUpload"
          image="imga"
          :data="{type:'Sku',width:800,height:800,size:2}"
        ></DraggleUpload>
      </div>
      <span
      ...
      <div class="detailBox">
        ...
        <DraggleUpload
          ...
          :data="{type:'Sku',width:760,size:2}"
        ></DraggleUpload>
      </div>
      <span
        style="margin-left: 180px;font-size:12px"
      >一张长条图，支持格式JPG/JPEG/PNG，详情图建议上传一张长条图；多图将按图片顺序展示,图片宽度760px，大小建议不超过2MB</span>
      <div class="footerBox">
...
import { addURL, removeID } from "@/utils/index";
...
export default {
  ...
  activated() {
      ...
      getPictureBanner(this.goodsPictureID).then(res => {
        if([...res.data.mainPictureFile,...res.data.slidePictureFile].length){
         this.$refs.DraggleUpload.init([
          ...res.data.mainPictureFile,
          ...res.data.slidePictureFile
        ]);
  ...
  methods: {
    ...
    openWin() {
      this.close({
        ...
        to: {
          name: "EditerParams",
          query: { oldID: this.goodsPictureID, goodsID: this.goodsPictureID },
    ...
    toDraft() {
      if (
        this.skuImageInfoParam.mainPictureFileId &&
        this.skuImageInfoParam.slidePictureFileIdList.length > 1
      ) {
