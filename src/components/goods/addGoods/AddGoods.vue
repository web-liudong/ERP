...
<template>
  <div class="addGoodsBox">
    ...
    <div class="w mainBox">
      ...
      >
        <el-form-item label="选择分类：" prop="name" class="sousuoBox">
          <!--  <el-input v-model="options2" placeholder="分类搜索"></el-input>-->
          <el-select
            v-model="options2"
            filterable
            remote
        ...
        <el-form-item label="采购价（含税）：" prop="buyingPrice">
          <el-input v-model="SkuBasicInfoParam.buyingPrice" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="友商售价：" prop="competingSkuPrice">
          <el-input v-model="SkuBasicInfoParam.competingSkuPrice" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        ...
        <el-form-item label="建议零售价：" prop="sellingPrice">
          <el-input v-model="SkuBasicInfoParam.sellingPrice" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="官网售价（含税）：" prop="websitePrice">
          <el-input v-model="SkuBasicInfoParam.websitePrice" placeholder="0.00">
            <template slot="append">元</template>
          </el-input>
        </el-form-item>
        <el-form-item label="销售单位：" prop="unit">
          <!--    <el-input v-model="SkuBasicInfoParam.unit" placeholder="输入单位/下拉查找"></el-input>-->
          <el-select
        ...
        <el-form-item label="保质期：" prop="shelfLife">
          <!-- <el-radio v-model="radioValue" label="1">有</el-radio>
          <el-radio v-model="radioValue" label="2">无</el-radio>-->
        ...
        <el-form-item label="条形码：">
          <el-input v-model="SkuBasicInfoParam.barcode" placeholder="填写条形码"></el-input>
        </el-form-item>
        ...
        <el-form-item label="集团主管/经理账号：" prop="managerDataAuthorityId">
          <el-select
            v-model="SkuBasicInfoParam.managerDataAuthorityId"
            filterable
            remote
            reserve-keyword
            placeholder="1~50个字以内"
        ...
        <el-form-item label="集团专员姓名：" prop="name7">
          <el-input v-model="commissionerRealName" placeholder="1~50个字以内" readonly></el-input>
        </el-form-item>
        ...
        <div v-for="(item,index) in SkuBasicInfoParam.organizationContacts"
          :key="item.key" >
         <el-form-item
            class="rules"
          ...
          >
            ...
            <el-input v-model="item.name" placeholder="输入一个旧商品ID" style="width: 400px" @input="item.name=item.name.replace(/[^\d]/g,'')" @blur="getNameByOldId(item.name,index)" clearable></el-input>
               <el-button @click="addCorrespond()" v-if="index==0">新增</el-button>
          </el-form-item>
          <el-form-item
            class="rules"
            style="flex-direction:row;align-items:flex-start;"
          >
            <span slot="label">旧商品名称：&nbsp;&nbsp;&nbsp;</span>
               <el-input v-model="item.phone" placeholder="输入旧商品ID，自动查询名称" style="width:400px" disabled></el-input>
               <el-button @click="deleteRules(index)" v-if="index!=0" type="danger">删除</el-button>
          </el-form-item>   
        </div>  
    ...
    <goodsCompare ref="goodsCompare" @callback="diaCallback"></goodsCompare>
  </div>
</template>
...
export default {
  ...
  data() {
    ...
    return {
      draftOrAuditer: null, //用于判断当前点击的按钮是保存到草稿还是保存到下一步
      oneName: "",
      twoName: "",
      threeName: "",
      // 单位下拉框的值
      ...
      rules: {
        ...
        // invoiceName: [
        //   { required: true, message: "税收分类简称不能为空", trigger: "blur" },
        // ],
        taxCode: [
  ...
  methods: {
    clear(name) {
      this.$refs.SkuBasicInfoParam.clearValidate(name);
    },
    getNameByOldId(value,index){
      if(value){
       getNameByOldId(value).then(res=>{
        if(res.code=='000000'){
          if(res.data.length){
          this.SkuBasicInfoParam.organizationContacts[index].phone=res.data;
          }else{
           this.SkuBasicInfoParam.relateSkuInfo[index].phone=null;
         }
        }
     })
      }else{
    ...
    addCorrespond(){
    this.SkuBasicInfoParam.organizationContacts.push({name:'',phone:''})
    },
    deleteRules(index){
     this.SkuBasicInfoParam.organizationContacts.splice(index,1) 
    },
    get() {
    ...
    splicingGoods(){
     this.SkuBasicInfoParam.name =
        this.SkuBasicInfoParam.nameCN +
        " " +
        this.SkuBasicInfoParam.nameEN +
        " " +
        ...
        //字符串截取赋值给开票名称和开票规格型号
        this.changeName=this.SkuBasicInfoParam.nameCN+' '+this.SkuBasicInfoParam.nameEN+' '+this.SkuBasicInfoParam.shortName
        ...
        // if(reg2.test(this.SkuBasicInfoParam.nameOnInvoice)){
        //   this.$message({})
        // }
    ...
    getBrandName(value) {
      ...
      this.SkuBasicInfoParam.nameCN = str[0] && str[0].nameCN;
      this.SkuBasicInfoParam.nameEN = str[0] && str[0].nameEN;
      this.splicingGoods()
    },
    //三级的分类搜索
    ...
    remoteMethod(query) {
      if (query !== "") {
        this.loading1 = true;
        let aa = { name: query, categoryId: this.threeId };
    ...
    remoteMethodUsername(query) {
      if (query !== "") {
        ...
        getUsername(aa).then(res => {
          if (res.code == "000000") {
            console.log(res.data, 5484848496496);
    ...
    // //选中值发生变化时触发事件
    getUSERNAME(ee) {
      let ggg = JSON.parse(JSON.stringify(this.options6));
      ggg = ggg.filter(item => {
        return item.id == ee;
      });
      this.realName = ggg[0] && ggg[0].realName;
      this.clear("managerDataAuthorityId");
    },
    ...
    getUSERNAMECOMMISS(ee) {
      let fff = JSON.parse(JSON.stringify(this.options7));
      fff = fff.filter(item => {
        return item.id == ee;
      });
      this.commissionerRealName = fff[0] && fff[0].realName;
      this.clear("staffDataAuthorityId");
    },
    //获取分类下拉一级分类信息
    getOneSelect() {
    ...
    submitForm(formName) {
      ...
      this.SkuBasicInfoParam.relatedSkuIdList=this.SkuBasicInfoParam.organizationContacts.filter((item)=>{
        return item.name
      }).map(item=>{
        return item.name
      })
      // this.SkuBasicInfoParam.relatedSkuIdList=this.SkuBasicInfoParam.organizationContacts.map(item=>{
      //   return item.name
      // })
      this.SkuBasicInfoParam.categoryId = this.threeId;
        ...
        if (valid) {
          recordGoodsAdd(this.checkSkuNameRepeatedParam).then(res => {
            ...
            if (res.data.length != 0) {
              ...
              //验证通过，保存至草稿
              createSkuBasicInfo(this.SkuBasicInfoParam).then(res => {
                if (res.code === "000000") {
                  this.$message({
                    message: "保存至草稿成功",
                    type: "success"
    ...
    diaCallback() {
      if (this.draftOrAuditer == "保存到草稿") {
        ...
        createSkuBasicInfo(this.SkuBasicInfoParam)
          .then(res => {
            if (res.code === "000000") {
              this.$message({
                message: "保存至下一步成功",
