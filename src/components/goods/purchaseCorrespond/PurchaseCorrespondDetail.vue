  ...
  <div class="detaillBox">
    <div class="detaillTable">
      ...
      <table border="1" v-if="tableData!=null">
        <tr>
          <th>新采购人编号：</th>
          <td>{{tableData.newUserNo}}</td>
          <th>新采购人名称：</th>
          <td>{{tableData.newUserName}}</td>
        </tr>
        <tr>
          <th>关联旧采购人ID：</th>
          <td>{{tableData.oldUserId}}</td>
          <th>
            <i>*</i> 旧采购人名称：
          </th>
          <td style="max-width:500px">{{tableData.oldUserName}}</td>
        </tr>
      </table>
    </div>
...
<script>
import { findProjectCorrespondDetail } from "@/api/goods/purchaseCorrespondManage";
...
export default {
  ...
  methods: {
    closed() {
      this.close({
        name: "PurchaseCorrespondDetail",
        to: { name: "PurchaseCorrespondManage", params: { isUpdate: true } }
      });
    },
    getDetail() {
