  ...
  <div class="detaillBox">
    <div class="detaillTable">
      ...
      <table border="1" v-if="tableData!=null">
        <tr>
          <th>新部门编号：</th>
          <td>{{tableData.newDepartmentNo}}</td>
          <th>新部门名称：</th>
          <td>{{tableData.newDepartmentName}}</td>
        </tr>
        <tr>
          <th>关联旧部门ID：</th>
          <td>{{tableData.oldDepartmentId}}</td>
          <th>
            <i>*</i> 旧部门名称：
          </th>
          <td style="max-width:500px">{{tableData.oldDepartmentName}}</td>
        </tr>
        <!-- <tr>
          <th>旧部门类型：</th>
          <td colspan="3">{{tableData.oldProjectTypeText}}</td>
        </tr> -->
      </table>
    </div>
...
<script>
import { findProjectCorrespondDetail } from "@/api/goods/sectionCorrespondManage";
...
export default {
  ...
  methods: {
    closed() {
      this.close({
        name: "SectionCorrespondDetail",
        to: { name: "SectionCorrespondManage", params: { isUpdate: true } }
      });
    },
    getDetail() {
