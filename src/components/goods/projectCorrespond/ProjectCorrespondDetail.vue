  ...
  <div class="detaillBox">
    <div class="detaillTable">
      ...
      <table border="1" v-if="tableData!=null">
        <tr>
          <th>新项目编号：</th>
          <td>{{tableData.newProjectNo}}</td>
          <th>新项目名称：</th>
          <td>{{tableData.newProjectName}}</td>
        </tr>
        <tr>
          <th>关联旧项目ID：</th>
          <td>{{tableData.oldProjectId}}</td>
          <th>
            <i>*</i> 旧项目名称：
          </th>
          <td style="max-width:500px">{{tableData.oldProjectName}}</td>
        </tr>
        <tr>
          <th>旧项目类型：</th>
...
<script>
import { findProjectCorrespondDetail } from "@/api/goods/projectCorrespondManage";
...
export default {
  ...
  methods: {
    closed() {
      this.close({
        name: "ProjectCorrespondDetail",
        to: { name: "ProjectCorrespondManage", params: { isUpdate: true } }
      });
    },
    getDetail() {
