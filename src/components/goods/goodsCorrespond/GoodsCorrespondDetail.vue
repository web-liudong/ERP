  ...
  <div class="detaillBox">
    <div class="detaillTable">
      ...
      <table border="1" v-if="tableData!=null">
        <tr>
          <th>新商品编号：</th>
          <td>{{tableData.newSkuNo}}</td>
          <th>新商品名称：</th>
          <td>{{tableData.newSkuName}}</td>
        </tr>
        <tr>
          <th>关联旧商品ID：</th>
          <td>{{tableData.oldSkuId}}</td>
          <th>
            <i>*</i> 旧商品名称：
          </th>
          <td style="max-width:500px">{{tableData.oldSkuName}}</td>
        </tr>
      </table>
    </div>
...
<script>
import { findCorrespondDetail } from "@/api/goods/goodsCorrespondManage";
...
export default {
  ...
  methods: {
    closed() {
      this.close({
        name: "GoodsCorrespondDetail",
        to: { name: "GoodsCorrespondManage", params: { isUpdate: true } }
      });
    },
    getDetail() {
    findCorrespondDetail(this.ID).then(res => {
