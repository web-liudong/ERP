  ...
  <div class="addGoodsBox">
    ...
    <div class="w mainBox">
      <el-form
        ...
        :rules="rules"
      >
        <el-form-item label="选择分类：" class="sousuoBox">
          <el-select
            v-model="options2"
            filterable
            remote
        ...
        <div v-for="(item,index) in SkuBasicInfoParam.relateSkuInfo"
         ...
         <el-form-item
          ...
          >
            ...
            <el-input v-model="item.relatedId" placeholder="输入一个旧商品ID" style="width: 400px" @input="item.relatedId=item.relatedId.replace(/[^\d]/g,'')" @blur="getNameByOldId(item.relatedId,index)"></el-input>
...
export default {
  ...
  data() {
    ...
    return {
      ...
      rules: {
        ...
        shortName: [
          { required: true, message: "商品自定义名称不能为空", trigger: "blur" }
        ],
        name: [
        ...
        warranty: [
          { required: true, message: "质保不能为空", trigger: "blur" }
        ],
        packingList: [
  ...
  activated() {
      ...
      this.EditerGoodsId = this.$route.query.copyID;
  ...
  methods: {
    ...
    getForm() {
      copyGoodsGetGoodsData(this.EditerGoodsId).then(res => {
        this.SkuBasicInfoParam=Object.assign(this.SkuBasicInfoParam,res.data)
        // this.SkuBasicInfoParam = res.data;
      ...
      if (query !== "") {
        ...
        getClassifyRemote({ condition: query }).then(res => {
          ...
          if (res.code == "000000") {
            ...
            console.log(aaaaaa, 999999999999);
    ...
    getUSERNAME(ee) {
      if (!ee) {
        ...
        let aaaa = JSON.parse(JSON.stringify(this.options6));

        aaaa = aaaa.filter(item => {
          return item.userName == ee;
        });
        this.managerData.id = aaaa[0].id;
        this.managerData.realName = aaaa[0].realName;
    ...
    getUSERNAMECOMMISS(ee) {
      let jjj = [];
        ...
        if(jjj.length){
        this.SkuBasicInfoParam.staffDataAuthorityId = jjj[0].id;
        this.SkuBasicInfoParam.staffDataAuthorityName = jjj[0].realName;
        }
    },
    ...
    submitForm(formName) {
      ...
      //   return item.relatedId
      // })
      console.log(this.SkuBasicInfoParam.relatedSkuIdList,5555)  
      if (this.radioValue == 2) {
        this.SkuBasicInfoParam.shelfLife = 0;
      }
        ...
        if (valid) {
          recordGoodsAdd(this.checkSkuNameRepeatedParam).then(res => {
            if (res.data.length != 0) {
              ...
              copyGoodsData(this.EditerGoodsId, this.SkuBasicInfoParam).then(
                res => {
                  console.log(res);
                  if (res.code === "000000") {
                    this.$message({
    ...
    disCallback() {
      if (this.draftOrAuditer == "保存到草稿") {
        //验证通过，保存至草稿
        copyGoodsData(this.EditerGoodsId, this.SkuBasicInfoParam).then(res => {
          console.log(res);
          if (res.code === "000000") {
            this.$message({
              message: "保存至草稿成功",
        ...
        copyGoodsData(this.EditerGoodsId, this.SkuBasicInfoParam)
          .then(res => {
            if (res.code === "000000") {
              this.$message({
                message: "保存至下一步成功",
          ...
          .then(res => {
            console.log(this.ID, "99999999999999999999999999");
            this.close({
              name: "copyGoods",
              to: {
                name: "EditerParams",
