...
<div class="correspondBox">
  <el-dialog title="新增关联关系" :visible.sync="isShow" width="30%" :close="isShowCallback">
      <el-form ref="correspondRef" :rules="personPhone" :model="query">
      <div class="contentBox" style="font-weight:bold;font-size:16px">新采购人</div>
      <el-form-item prop="" class="rules" style="flex-direction: row;align-items: flex-start;">
       <span slot="label">新采购人ID：</span>
       <el-input type="text" style="width: 60%" v-model="query.des" placeholder="输入一个新采购人ID" @input="query.des=query.des.replace(/[^\d]/g,'')" @blur="getNewGoods" clearable></el-input>
      </el-form-item>
      <div style="margin-top:10px;height:100%;word-wrap: break-word; word-break: normal; ">新采购人名称：<span style="color:#339999">{{query.newGoodsName}}</span></div> 
      <div style="margin-top:15px;" >
        <div class="contentBox" style="font-weight:bold;font-size:16px;">关联旧采购人  <el-button size="small" type="text" @click="addCorrespond">
            添加关联采购人
            </el-button>
        </div> 
        ...
        <div v-for="(item,index) in query.organizationContacts"
         ...
         <el-form-item
          ...
          >
            <span slot="label" style="width:150px">旧采购人ID：</span>
            <el-input v-model="item.name" placeholder="输入一个旧采购人ID" style="width: 60%" @input="item.name=item.name.replace(/[^\d]/g,'')" @blur="getOldName(item.name,index)"></el-input>
          </el-form-item>
          <el-form-item
            class="rules"
            style="flex-direction:row;align-items:flex-start;"
          ...
          >
            <span slot="label" style="width:150px">旧采购人名称：</span>
               <el-input v-model="item.phone" placeholder="输入旧采购人ID，自动查询名称" style="width: 60%" disabled></el-input>
    ...
    getNewGoods(){
      if(this.query.des){
       searchProjectInfo(this.query.des).then(res=>{
         this.query.newGoodsName=res.data.userName;   
         this.query.newGoodsId=res.data.userId;   
      })
      }
    },
     ...
     if(value){
      getNameByOldId(value).then(res=>{
         ...
         this.query.organizationContacts[index].phone=res.data  
             ...
             this.$confirm(`不存在旧采购人ID:${value},请核查`,'错误提示',{
