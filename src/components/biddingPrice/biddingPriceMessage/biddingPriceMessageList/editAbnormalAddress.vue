<template>
  <div class="form-wrapper">
    <addhead headname="未识别地址信息：" iconName="iconjinridingdanshu"></addhead>
    <div class="address">
      <el-row>
       <el-col :span="24"><span>收货地址:</span><p>{{form.errorAddress}}</p></el-col>
      </el-row>
    </div>
    ...
    <div>
      <el-form ref="formName" :rules="rules" :model="form" class="form-box">
        <el-form-item>
          <el-row>
            <el-col :span="8">
              <el-form-item label="项目：" label-width="135px" class="forms" prop="projectId">
                <el-select
          ...
          <el-row>
            <el-col :span="16">
              <el-form-item label="收货地址：" label-width="135px" class="forms address-input" prop="provinceId">
            ...
            <el-col :span="8">
              <el-form-item label="外部竞价编号：" label-width="135px" class="forms" prop="exteriorNo">
                <el-input  v-model="form.exteriorNo" style="width: 200px" disabled></el-input>   
              </el-form-item>
            </el-col> 
          </el-row>
          <el-row>
    ...
    <div class="button-box">
      <el-button type="primary" @click="editBiddingPriceInfo('formName')">保存并推送</el-button>
      ...
      <el-form ref="addressName" :rules="formRules" :model="formAddress">
        <div class="address-box">
          <el-form-item label="收货地址：" label-width="110px" class="forms addressBox" prop="provinceName">
            <el-select v-model="formAddress.provinceName" placeholder="选择省/直辖市" @change="onProvincesChange" style="width:200px">
              <el-option
          ...
          <el-form-item  prop="cityName">
            <el-select v-model="formAddress.cityName" placeholder="选择城市" @change="onCityChange"  style="width:200px">
              <el-option
          ...
          <el-form-item prop="countyName">
            <el-select v-model="formAddress.countyName" placeholder="选择区/县" @change="onCountyChange"  style="width:200px">
              <el-option
        ...
        <el-form-item label="详情地址：" label-width="110px" class="forms addressBox" prop="addressLine">
          <el-input type="textarea"  v-model="formAddress.addressLine" class="address-textarea"></el-input>   
        </el-form-item>
        <el-form-item class="forms addressBox" style="text-align: center;">
          ...
          <el-button type="primary" @click="oncancel('addressDialog')">取消</el-button>  
        </el-form-item>
      </el-form>
    </el-dialog>
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="warningEditBiddingPriceInfo('formName')" v-if="saveDialogTile=='警告'">确定</el-button>
...
<script>
  ...
  import {
    editBiddingPriceInfo,
    getServiceParty,
    getBiddingEndTime,
    editEchoBiddingPriceInfo,
    warningEditBiddingPriceInfo
  ...
  export default {
    ...
    data() {
      ...
      return {
        ...
        rules:{
          ...
          address:[
             { required:true, message: '请输地址', trigger:'input'}
          ],
          provinceId:[
    ...
    activated(){
      if(this.biddingId!=this.$route.query.id){
        this.editEchoBiddingPriceInfo();
      }
    },
    methods: {
      editEchoBiddingPriceInfo(id){
        editEchoBiddingPriceInfo(id).then(res=>{
          if(res.code=='000000'){
            this.form=res.data;
            this.form.address='';
            this.form.createBiddingSkuParamList=res.data.biddingSkuDTOs;
            if(res.data.provinceName!=null){
              this.form.address=res.data.provinceName+'-'+res.data.cityName+'-'+res.data.countyName+'-'+res.data.addressLine;
              this.formAddress.provinceName=res.data.provinceName;
              this.formAddress.cityName=res.data.cityName;
              this.formAddress.countyName=res.data.countyName;
              this.formAddress.addressLine=res.data.addressLine;
            }
          }
        })
      },
      ...
      querySearchAsync(queryString, cb) {
        pnameList({
          ...
          name:this.form.customerNameCustomize
        }).then(res=>{
            if(res.code=='000000'){
      ...
      editBiddingPriceInfo(formName){
        this.$refs[formName].validate((valid) => {
          if(valid) {
            editBiddingPriceInfo(this.biddingId,this.form).then(res=>{
              if(res.code=='000000'){
                this.$message({
                  message: '保存成功',
                  type: 'success'
                ...
                this.$refs[formName].clearValidate();
                 this.timeout = setTimeout(() => {
                    this.close({name:'biddingPriceMessageEditAbnormalAddress', to: {name:'biddingPriceMessageList', params:{isUpdate:true}}});
                  }, 3000 * Math.random());
              }
            }).catch(rej=>{
              if(rej.response.data.code=='010210004'){
                this.saveDialogTile='错误提示';
                this.saveDialogTips=`该竞价信息（${this.form.projectId} ${this.form.exteriorNo}）已存在，不可重复添加`;
      ...
      warningEditBiddingPriceInfo(formName){
        this.$refs[formName].validate((valid) => {
          if(valid) {
            warningEditBiddingPriceInfo(this.biddingId,this.form).then(res=>{
              if(res.code=='000000'){
                this.$message({
                  message: '保存成功',
                  type: 'success'
