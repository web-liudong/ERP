    ...
    <div class="createBox">
      ...
      <div class="infoBox">
        ...
        <div class="flex" style="width:50%">
          ...
          <el-form-item required prop="addressLine">
            <el-input
            ...
            <div
              ...
              style="cursor: pointer; width:600px;vertical-align:middles;display: inline-block;border: 1px solid #dcdfe6;height: 40px;border-radius: 4px;box-sizing: border-box;padding: 0 15px;overflow: hidden; text-overflow:ellipsis;white-space:nowrap"
            >
              <span>{{form.detailAddressLine}}</span>
            </div>
          </el-form-item>
        </div>
        <div class="module">
          <div class="flex">
            ...
            <el-form-item prop="exteriorNo">
              <el-input v-model="form.exteriorNo" style="max-width: 200px;" disabled></el-input>
            </el-form-item>
          </div>
          <div class="flex">
            <span>
...
<script>
import setAddress from "../../../order/child/set_address";
...
export default {
  ...
  mounted() {
    ...
    this.organizationName = userPosts[0].organizationName;
  ...
  methods: {
    ...
    getEcho() {
      reporeEditEcho(this.$route.query.id).then(res => {
        if ((res.code = "000000")) {
          this.form = res.data;
          // this.form.priceQuote=parseInt( this.form.priceQuote)
          console.log(this.form.priceQuote);
          console.log(typeof this.form.priceQuote);

          console.log(this.form.provinceId);
          console.log(typeof this.form.provinceId);

          this.receiptAddress = res.data.detailAddressLine;
          this.receiptAddress = {
            province: { id: res.data.provinceId },
            city: { id: res.data.cityId },
            county: { id: res.data.countyId },
            address: res.data.addressLine,
            receiptAddress: res.data.detailAddressLine
          };
        }
      });
    },
    ...
    subMit(formName) {
      ...
      if (ro.skuName && ro.qty) {
        reporeEdit(this.$route.query.id, this.form)
          .then(res => {
            if ((res.code = "000000")) {
              console.log(res.data);
          ...
          .catch(rej => {
            ...
            // if ((rej.response.code = "010210024")) {
            //   this.diallodAlreport = true;
            // }
            // else if((rej.response.code = "010210028")) {
            //   this.dia = true;
            // }
            // else if((rej.response.code = "010210027")) {
            //   this.dialogBlock = true;
            // }
          });
      } else {
    ...
    two(row, b) {
      if (b == "qty") {
        row.qty = Math.round(row[b]);
      }
      if (b == "price") {
        row.price = returnFloat(row[b]);
      }
    }
  }
};
