<template>
  <el-dialog title="收货地址" :visible.sync="isShow" width="650px" append-to-body>
    <el-row>
      <el-col>
...
export default {
  ...
  methods: {
    init(row) {
      ...
      this.form = {
        province: { id: row.recipientAddressProvinceId },
        city: { id: row.recipientAddressCityId },
        county: { id: row.recipientAddressCountyId },
        address: row.recipientAddressLine,
        receiptAddress: row.recipientAddress
      };
      this.getProvinces();
      if (this.initObject.recipientAddressProvinceId) {
        this.getcity(this.form.province, true);
      }
      if (this.initObject.recipientAddressCityId) {
        this.getcounty(this.form.city, true);
      }
      this.isShow = true;
    },
    getProvinces() {
    ...
    getcity(provinces, isInit) {
      ...
      subs(provinces.id).then(res => {
        if (res.code == "000000") {
          this.city = res.data;
          if (isInit) {
            this.form.city = { id:this.initObject.recipientAddressCityId};
          }
        }
      });
    },
    getcounty(city, isInit) {
      ...
      subs(city.id).then(res => {
        if (res.code == "000000") {
          ...
          if (isInit) {
            this.form.county = { id:this.initObject.recipientAddressCountyId};
          }
        }
      });
    },
      ...
      this.$refs[name].validate(valid => {
        if (valid) {
          ...
          if(this.form.province.name){
            ...
            let arr = this.initObject.recipientAddress.split("-");
            ...
            this.form.receiptAddress = this.initObject.recipientAddress ? this.initObject.recipientAddress.replace(this.initObject.recipientAddressLine, this.form.address) : ''
          }
          this.$emit("callback", {type:"setAddress", data:this.form});
        } else {
