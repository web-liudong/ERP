<template>
  <el-dialog title="发布确认" :visible.sync="isShow" class="releaseTable">
    <el-table
      ...
      max-height="400"
    >
      <el-table-column prop="skuName" label="对象名称" align="center"></el-table-column>
...
<script>
import { opaShoopeRelease } from "@/api/release/specialProjectManage";
export default {
  props: {},
  data() {
    return {
  ...
  methods: {
    ...
    getDataList() {
      opaShoopeRelease(this.shoppeId, "get").then(res => {
    ...
    submitForm(name) {
      opaShoopeRelease(this.shoppeId, "put").then(res => {
        if (res.code == "000000") {
          this.isShow = false;
          this.$emit("callback", "shoppeRelease");
        }
      });
    }
  }
