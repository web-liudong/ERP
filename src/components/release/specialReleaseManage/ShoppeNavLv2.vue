    ...
    <div class="listBox">
      <div class="listBoxTop">
        ...
        <div class="listBoxRight">
          <div class="button-head" @click="oepnWin('ShoppeAddNav',{ id: shoppeId, shoppe: shoppeName, navId: navParentId, level: 2 })">
      ...
      <el-table :data="tableData" border style="width: 100%" size="small" :max-height="dataListHeight">
        ...
        <el-table-column label="相关" align="center" width="160px">
          <template slot-scope="scope">
            <el-button
              @click="oepnWin('ShoppeNavLv3',{ id: shoppeId, shoppe: shoppeName, navId: scope.row.id, source: 2 })"
              type="text"
              size="small"
            >三级({{scope.row.thirdLevelCount}})</el-button>
            <el-button
              @click="oepnWin('ShoppeGoodsManage',{ id: shoppeId, shoppe: shoppeName, navId: scope.row.id, level:2 })"
              type="text"
              size="small"
        ...
        <el-table-column label="操作" align="center" width="220px">
          <template slot-scope="scope">
            ...
            <el-button @click="oepnWin('ShoppeEditerNav',{ id: scope.row.id, level: 2 })" type="text" size="small">编辑</el-button>
            <el-button @click="oepnWin('ShoppeNavDetail',{ id: scope.row.id, level: 2 })" type="text" size="small">详情</el-button>
          </template>
        </el-table-column>
      </el-table>
      <div class="block">
...
export default {
  ...
  activated(){
    if(this.$route.params.isUpdate || this.$route.query.id != this.shoppeId || this.$route.query.navId != this.navParentId){
      this.init();
    }
  },
  computed:{
  ...
  methods: {
    init(){
      ...
      this.query.firstLevelId = this.$route.query.navId;
      //获取表格数据
      this.getTableData();
    },
    //获取表格数据
    getTableData() {
      getNavList(this.shoppeId, "second", this.query).then(res => {
    ...
    querySearch(query, cb) {
      if (query) {
        ...
        queryNavList({ name: query, level: "SECOND_LEVEL" }).then(res => {
