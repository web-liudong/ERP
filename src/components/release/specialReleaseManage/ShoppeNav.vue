<template>
  <div class="brandBox">
    ...
    <div class="listBox">
      <div class="listBoxTop">
        ...
        <div class="listBoxRight">
          <div
            class="button-head"
            @click="showDialog('setReleaseNav',{shoppeId:shoppeId})"
            v-if="$authorities.ShoppeNavReleaseRelevant"
          >
          ...
          <div
            class="button-head"
            @click="oepnWin('ShoppeAddNav',{ id: shoppeId, shoppe: shoppeName, level: 1 })"
            v-if="$authorities.ShoppeNavAdd"
          >
            <i class="icontianjiashangpin iconfont button-head-icon" style="font-size:14px;"></i>添加
          </div>
          <div class="button-head" @click="showDialog('importFile')" v-if="$authorities.ShoppeNavImportRelevant">
            <i class="icondaoru iconfont button-head-icon"></i>导入
          </div>
          <div
            class="button-head"
            @click="oepnWin('SysReleaseRecord',{ type: 'navigation-release-history', id: shoppeId })"
            v-if="$authorities.ShoppeNavReleaseRelevant"
          >
          ...
          <div
            class="button-head"
            @click="oepnWin('SysImportRecord',{ type: 'release-nav-import-list', id: shoppeId })"
            v-if="$authorities.ShoppeNavImportRelevant"
          >
      ...
      <el-table :data="tableData" border style="width: 100%" size="small" :max-height="dataListHeight">
        ...
        <el-table-column label="相关" align="center" width="220px">
          <template slot-scope="scope">
            <el-button
              @click="toOtherNav(2,scope.row)"
              type="text"
              size="small"
            >二级({{scope.row.secondLevelCount}})</el-button>
            <el-button
              @click="toOtherNav(3,scope.row)"
              type="text"
              size="small"
            >三级({{scope.row.thirdLevelCount}})</el-button>
            <el-button
              @click="oepnWin('ShoppeGoodsManage',{ id: shoppeId, shoppe: shoppeName, navId: scope.row.id, level:1 })"
              type="text"
              size="small"
        ...
        <el-table-column label="操作" align="center" width="220px">
          <template slot-scope="scope">
            <el-button
              @click="oepnWin('ShoppeRelevantGoodsManage',{ shoppeId: shoppeId, shoppe: shoppeName, id: scope.row.id })"
              type="text"
              size="small"
              v-if="$authorities.ShoppeNavRelevantGoods"
            >关联商品管理</el-button>
            <el-button
              @click="opaUseStatus(scope.row)"
              type="text"
              size="small"
              v-if="$authorities.ShoppeNavForbidden && scope.row.available"
            >禁用</el-button>
            <el-button
              @click="opaUseStatus(scope.row)"
              type="text"
              size="small"
              v-if="$authorities.ShoppeNavAvailable && !scope.row.available"
            >启用</el-button>
            <el-button
              @click="oepnWin('ShoppeEditerNav',{ id: scope.row.id, level: 1 })"
              type="text"
              size="small"
              v-if="$authorities.ShoppeNavEdit"
            >编辑</el-button>
            <el-button
              @click="oepnWin('ShoppeNavDetail',{ id: scope.row.id, level: 1 })"
              type="text"
              size="small"
              v-if="$authorities.ShoppeNavDetail"
            >详情</el-button>
          </template>
        </el-table-column>
      </el-table>
    ...
    <setReleaseNav ref="setReleaseNav" @callback="dialogCallback"></setReleaseNav>
    <importFile
      ref="importFile"
      :title="'导航信息批量导入'"
      :type="importType"
...
import setReleaseNav from "./child/set_release_nav";
...
export default {
  ...
  activated(){
    if(this.$route.params.isUpdate || this.$route.query.id != this.shoppeId){
      this.init();
    }
  },
  computed:{
  ...
  methods: {
    init(){
      ...
      //获取表格数据
      this.getTableData();
      this.getNavReleaseCount();
    },
    //获取表格数据
    getTableData() {
      getNavList(this.shoppeId, "first", this.query).then(res => {
        this.tableData = res.data.pageInfo.items;
        this.totalPage = res.data.pageInfo.total;
      });
    },
    getNavReleaseCount() {
      getNavReleaseCount(this.shoppeId).then(res => {
    ...
    querySearch(query, cb) {
      if (query) {
        ...
        queryNavList({ name: query, level: "FIRST_LEVEL" }).then(res => {
    ...
    toOtherNav(level, row) {
      if (level == 2) {
        this.$router.push({
          name: "ShoppeNavLv2",
          query: { id: this.shoppeId, shoppe: this.shoppeName, navId: row.id }
        });
      } else {
        this.$router.push({
          name: "ShoppeNavLv3",
