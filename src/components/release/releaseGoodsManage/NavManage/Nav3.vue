      ...
      <el-table :data="tableData" border :max-height="dataListHeight">
        ...
        <el-table-column prop="address" label="相关" align="center" width="350px">
          <template slot-scope="scope">
            <router-link
              :to="{name:'ReleaseGoodsManage',query: {nav3:scope.row.id}}"
            >
...
<script>
import { thirdList, forbidden, available } from "@/api/release/navManage";
export default {
  inject: ["reload"],
  name: "NavManage3",
  data() {
  ...
  created() {
    this.secondLevelId = this.$route.query.id2;
  ...
  methods: {
    ...
    addNew() {
      ...
      this.$router.push({
        name: "AddThird",
        query: {
          id1: that.firstLevelId,
          id2: that.secondLevelId
        }
      });
    },
    getpreview() {
