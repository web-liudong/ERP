...
<script>
import { nameList, brandList } from "@/api/release/navManage";
import { skuList2, skudel, skuTop } from "@/api/release/floorManage";
...
export default {
  ...
  methods: {
    ...
    getData(page) {
      skuList2(this.floorId, this.navId, {
    ...
    top(id) {
      skuTop(this.floorId, this.navId, id).then(res => {
        if (res.code == "000000") {
          this.$message({
            type: "success",
            message: "商品已置顶"
    ...
    del(id) {
      this.$confirm(
        "删除之后，此项目前台不再展示此商品，列表不在显示此条数据，确定要删除吗？",
        "删除提示",
        {
        ...
        skudel(this.floorId, this.navId, id).then(res => {
          if (res.code == "000000") {
            this.$message({
              type: "success",
              message: "删除商品已成功"
