...
<script>
  import {forAdditionFloor,navigationsku,navigationskus,nameList,brandList} from '@/api/release/navManage'
  import {addSkuFloor,addSkusFloor} from '@/api/release/floorManage'
  ...
  export default {
    ...
    methods: {
      ...
      addLink(id){
        this.$confirm('确定将此商品添加到楼层推荐中吗？', '添加提示', {
          confirmButtonText: '确定',
          cancelButtonText: '取消',
        }).then(() => {
          addSkuFloor(this.floorId,this.navId,id).then(res=>{
            if(res.code=='000000'){
              this.$message({
                type: 'success',
                message: '商品添加成功'
      ...
      addAll(){
        if(this.selectionku.length){
            ...
            this.$confirm('确定将此将此商品关联导航吗？', '添加提示', {
              ...
              addSkusFloor(this.floorId,this.navId,checkArr
              ).then(res=>{
                if(res.code=='000000'){
                  this.$message({
                    type: 'success',
